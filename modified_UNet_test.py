...
def adjust_learning_rate(optimizer, epoch, init_lr):
    ...
    lr = init_lr * 0.99 ** (epoch-1)#args.lr * (0.1 ** (epoch // 30))
...
def train(train_loader, valid_loader, loss_type, act_type, tolerance, result_path, log_interval=10, lr=0.000001,
          max_epochs=500):
    ...
    best_global_model_path = result_path + '/best_global_model.pth'
    ...
    for epoch in range(1, max_epochs + 1):
        for phase in ['train', 'val']:
            ...
            for batch_i, sample in enumerate(loader):
                ...
                if batch_i % log_interval == 0:
                    print('{} Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(phase,
                                                                                epoch, batch_i * len(data),
                                                                                len(loader.dataset),
                                                                                100. * batch_i / len(loader),
                                                                                loss.item()))
            ...
            if last_avg_loss < global_minimum_loss:
                global_minimum_loss = last_avg_loss
                # --------------------- Saving the best found model -----------------------
                torch.save(model.state_dict(), best_global_model_path)
            if phase == 'train':
                train_last_avg_loss = last_avg_loss
            ...
            if phase == 'val':
                valid_last_avg_loss = last_avg_loss#np.mean(all_batches_losses)
...
if __name__ == '__main__':
    ...
    obj = {'batch_size': batch_size, 'max_epochs': max_epochs, 'valid_portion': valid_portion, 'shuffle': shuffle,
    ...
    # load train dataset
    GlaS_train_dataset = GlaSDataset(transform=transformations, desired_dataset='train', data_expansion_factor=8)

    # load valid dataset
    GlaS_valid_dataset = GlaSDataset(transform=transformations, desired_dataset='val', data_expansion_factor=8)
    ...
    train(train_loader, valid_loader, loss_type, act_type, tolerance, result_path, log_interval=10, lr=lr,
